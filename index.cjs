...
function getSession(roomId) {
  if (!userSessions.has(roomId)) {
    userSessions.set(roomId, { lang: 'es', industry: null, profession: null, waitingFor: null, greeted: false });
  }
...
const T = {
  es: {
    ...
    askProfession: '¿A qué te dedicas o cuál es tu profesión?\n\nEj: contador, médico, abogado, desarrollador, emprendedor...',
    ...
    profSaved:   '✅ Anotado: **{p}**. Usaré eso para personalizar los resultados.',
  ...
  en: {
    ...
    askProfession: 'What do you do or what\'s your profession?\n\nEx: accountant, doctor, lawyer, developer, entrepreneur...',
    ...
    profSaved:   '✅ Noted: **{p}**. I\'ll use that to personalize results.',
...
async function sendMenu(roomId, lang) {
  ...
  // Etiqueta del botón de profesión: muestra la guardada o texto genérico
  const profLabel = s.profession
    ? '👤 ' + s.profession
    : (isEs ? '👤 Mi profesión' : '👤 My profession');
  await agent.sendReplyMarkupMessage('buttons', roomId, t(lang, 'menuLabel'), [
    ...
    [
      { text: profLabel,                                                  callback_data: 'ACTION:profession'  },
...
function registerHandlers() {
  ...
  agent.addCommand('callback_query', async ({ message, roomId }) => {
    ...
    if (cmd === 'ACTION') {
      ...
      if (data === 'profession') { s.waitingFor = 'profession'; await agent.sendConnectionMessage(roomId, t(lang, 'askProfession')); return; }
    }
  ...
  agent.addCommand('message', async ({ message, roomId }) => {
    ...
    // ── Primera vez: saludo + botones de inmediato ────────────────────────────
    if (!s.greeted) {
      s.greeted = true;
      await agent.sendConnectionMessage(roomId, t(lang, 'welcome'));
      await sendMenu(roomId, lang);
      return;
    }
    ...
    if (s.waitingFor === 'profession') {
      ...
      s.industry   = s.industry || text.trim(); // si no tenía industria, la profesión la rellena
      s.waitingFor = null;
      await agent.sendConnectionMessage(roomId, t(lang, 'profSaved', { p: s.profession }));
      await sendMenu(roomId, lang);
      return;
    }
